...
async def weight_tracker_menu(message: types.Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📈 Показати графік", callback_data="weight_chart")],
        [InlineKeyboardButton(text="📋 Історія записів", callback_data="weight_history")]
    ])
...
@dp.callback_query(F.data == "weight_history")
async def weight_history_callback(callback: types.CallbackQuery):
    """Показ історії ваги"""
    user_id = str(callback.from_user.id)
    
    if user_id not in users_db or 'weight_history' not in users_db[user_id]:
        await callback.message.edit_text("📊 У вас ще немає записів ваги.")
        return
    
    history = users_db[user_id]['weight_history'][-10:]  # Останні 10 записів
    
    text = "📋 <b>Історія ваги (останні 10 записів):</b>\n\n"
    ...
    for i, record in enumerate(reversed(history), 1):
        ...
        if i > 1:
            prev_weight = list(reversed(history))[i-1]['weight']
            change = weight - prev_weight
            change_text = f" ({change:+.1f})" if change != 0 else ""
        else:
            change_text = ""
        
        text += f"{i}. <b>{weight} кг</b>{change_text}\n   📅 {date}\n\n"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_weight_menu")]
    ])
    
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(F.data == "back_to_weight_menu")
async def back_to_weight_menu(callback: types.CallbackQuery):
    """Повернення до меню ваги"""
    user_id = str(callback.from_user.id)
    progress = get_weight_progress(user_id)
...
async def food_diary_menu(message: types.Message):
    ...
    # Отримуємо калорії за сьогодні
    today_calories, today_entries = get_daily_calories(user_id)
    
    # Підрахунок рекомендованих калорій
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📋 Записи за сьогодні", callback_data="today_diary")],
        [InlineKeyboardButton(text="📈 Статистика тижня", callback_data="week_stats")]
    ])
...
@dp.callback_query(F.data == "today_diary")
async def today_diary_callback(callback: types.CallbackQuery):
    """Показ записів за сьогодні"""
    user_id = str(callback.from_user.id)
    today_calories, today_entries = get_daily_calories(user_id)
    
    if not today_entries:
        await callback.message.edit_text("📅 Сьогодні ще немає записів у щоденнику.")
        return
    
    text = f"📅 <b>Записи за сьогодні:</b>\n\n"
    
    for i, entry in enumerate(today_entries, 1):
        text += f"{i}. <b>{entry['food_name']}</b>\n"
        text += f"   ⚖️ {entry['weight']} г | 🔥 {entry['calories']} ккал\n"
        text += f"   🕐 {entry['time']}\n\n"
    
    text += f"📊 <b>Загалом: {today_calories} ккал</b>"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_diary_menu")]
    ])
    
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(F.data == "back_to_diary_menu")
async def back_to_diary_menu(callback: types.CallbackQuery):
    """Повернення до меню щоденника"""
    user_id = str(callback.from_user.id)
